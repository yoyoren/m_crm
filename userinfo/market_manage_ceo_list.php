...
$marketname=chkstring(addslashes(trim(isset($_POST['marketname'])?$_POST['marketname']:"")));
...
//								$gqquery ="SELECT  *  from market ORDER BY classcode desc,code"; 
								if ($marketname<>"")
								{
									$gqquery =$parentid==-1?"SELECT  *  from market where  marketname like '%".$marketname."%' ORDER BY addtime desc LIMIT ".$nowpage.","."$dbzz_bianma_pagenum":"SELECT  *  from market where userid in(select userid from userinfo where parentid=$departmentid) and  marketname like '%".$marketname."%' ORDER BY addtime desc LIMIT ".$nowpage.","."$dbzz_bianma_pagenum";
									}
									else
									{
										$gqquery =$parentid==-1?"SELECT  *  from market  ORDER BY addtime desc LIMIT ".$nowpage.","."$dbzz_bianma_pagenum":"SELECT  *  from market where userid in(select userid from userinfo where parentid=$departmentid)  ORDER BY addtime desc LIMIT ".$nowpage.","."$dbzz_bianma_pagenum"; 
										}
...
//				  			$gqquery ="SELECT  *  from market WHERE id NOT IN (SELECT TOP ".$nowpage." id from market ORDER BY classcode desc,code) ORDER BY classcode desc,code";  
					...
					if ($marketname<>"")
					{	
						$query0 =$parentid==-1?"SELECT id from market where marketname like '%".$marketname."%'":"SELECT id from market where userid in(select userid from userinfo where parentid=$departmentid) and marketname like '%".$marketname."%'";
						}
						else
						{
							$query0 =$parentid==-1?"SELECT id from market ":"SELECT id from market where userid in(select userid from userinfo where parentid=$departmentid) ";
							}
//							echo $gqquery;
//							exit;
...
if ($ggallrows==0)
{
	echo "<tr><td align='center' bgcolor='#FFFFFF' height='25' colspan='8'>暂无您要查询的数据！</td></tr>";
	}
